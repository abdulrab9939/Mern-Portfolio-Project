...
export const addworkexperience =
  (title, description, date1, date2) => async (dispatch) => {
    try {
      dispatch({
        type: "ADD_WORKEXPERIENCE_REQUEST",
      });
      ...
      dispatch({
        type: "ADD_WORKEXPERIENCE_SUCCESS",
        payload: data.message,
      });
    } catch (error) {
      dispatch({
        type: "ADD_WORKEXPERIENCE_FAILURE",
        payload: error.response.data.message,
      });
    }
  };
...
export const addAbout =
  (url, name, title, subtitle, description, quote, avatar) =>
  async (dispatch) => {
    try {
      dispatch({
...
export const addtechknow = (url, name) => async (dispatch) => {
  try {
    dispatch({
      type: "ADD_PROJECT_REQUEST",
    });
...
export const deleteTechknow = (id) => async (dispatch) => {
  try {
    dispatch({
      type: "DELETE_TECKNOW_REQUEST",
    });

    const { data } = await axios.delete(`/api/v1/admin/tecknow/${id}`);

    dispatch({
      type: "DELETE_TECHKNOW_SUCCESS",
      payload: data.message,
    });
  } catch (error) {
    dispatch({
      type: " DELETE_TECHKNOW_FAILURE",
      payload: error.response.data.message,
    });
  }
};
